...
if __name__=="__main__":
  ...
  if args.fixed_design and args.fixed_design_opt_reg:
    ...
    with open("/dfs/scratch0/zjian/lp_kernel/noise_trade_off/U_" + str(args.n_fp_rff) + ".npy", "w") as f:
      np.save(f, U)
    ...
    x0 = 1.0
    lambda_list_print = [] 
    for fixed_design_noise_level in [1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 2e5, 5e5, 1e6]:
      x0 = 1.0
      f = lambda lam: expected_loss(lam,U,S,Y_train_orig,fixed_design_noise_level)
      res = minimize(f, x0, bounds=[(0.0, None)], options={'xtol': 1e-6, 'disp': True})
      loss = f(res.x)
      print("fixed design opt reg and loss", res.x, loss)
      lambda_list_print.append(float(res.x))
    print("optimal lambda", lambda_list_print)
    exit(0)
