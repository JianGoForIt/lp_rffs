    ...
    for perc in 0.0 0.1 1.0 10.0
      do
        if [ ! -f "${output}_n_fp_feat_${n_fp_rff}_lp_pca_sqr_rff_n_base_feat_1024_auto_scale_perc_${perc}/results.pkl" ]; then
          ...
          python rff_kernel_census.py --pca_rff --pca_rff_auto_scale --pca_rff_perc=${perc} --n_fp_rff=${n_fp_rff} --pca_rff_n_base_fp_feat=1024 --sigma=${sigma} --random_seed=${seed} --reg_lambda=${lambda} --output_folder="${output}_n_fp_feat_${n_fp_rff}_lp_pca_sqr_rff_n_base_feat_1024_auto_scale_perc_${perc}"
        fi
      done
  done
    ...
    for perc in 0.0 0.1 1.0 10.0
      do
        if [ ! -f "${output}_n_fp_feat_${n_fp_rff}_lp_pca_sqr_rff_n_base_feat_8192_auto_scale_perc_${perc}/results.pkl" ]; then
          ...
          python rff_kernel_census.py --pca_rff --pca_rff_auto_scale --pca_rff_perc=${perc} --n_fp_rff=${n_fp_rff} --pca_rff_n_base_fp_feat=8192 --sigma=${sigma} --random_seed=${seed} --reg_lambda=${lambda} --output_folder="${output}_n_fp_feat_${n_fp_rff}_lp_pca_sqr_rff_n_base_feat_8192_auto_scale_perc_${perc}"
        fi
      done
  done
