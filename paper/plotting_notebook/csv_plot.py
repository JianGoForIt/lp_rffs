...
#         print x, average_y, std_y
        if "FP" in label:
            print "FP mode plot"
            plt.errorbar(x, average_y, yerr=std_y, label=label, marker=marker_list[i], markeredgecolor=color_list[i % len(color_list)], markerfacecolor="none", markersize=8, markeredgewidth=1.5, fmt="-", linewidth=1, capsize=5, capthick=1, color=color_list[i % len(color_list)])
        else:
            print "LP mode plot"   
            plt.errorbar(x, average_y, yerr=std_y, label=label, marker=marker_list[i], markeredgecolor=color_list[i % len(color_list)], markerfacecolor="none", markersize=8, markeredgewidth=1.5, fmt="--", linewidth=1, capsize=5, capthick=1, color=color_list[i % len(color_list)])
