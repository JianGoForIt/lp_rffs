...
\begin{figure}
	\centering
	\begin{tabular}{c c}
		\subfigure[Census heldout MSE]{\label{fig:year_pred_mem} \includegraphics[width=0.33\linewidth]{figures/regression_l2_vs_delta.pdf} } \hfill
		\subfigure[CovType heldout error]{\label{fig:year_pred_mem} \includegraphics[width=0.33\linewidth]{figures/classification_acc_vs_delta.pdf} } \hfill
	\end{tabular}
	\caption{The strong correlation between generalization performance and $\lambda$-spectral distance $D_{\lambda}(K,\tK)$ under memory budgets for the Census dataset (left) and subsampled CovType dataset (right).  Different kernel approximation approaches demonstrate similar generalization performance for similar $\lambda$-spectral distance.}
	\label{fig:specdist}
\end{figure}
