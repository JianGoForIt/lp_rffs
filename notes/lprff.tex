...
\textbf{$\eps$-log-spectral distance (Definition)}:  Given $\eps \geq 0$, and two element-wise non-negative vectors $\sigma,\tsigma \in \RR^n$,  we define the $\eps$-log-spectral distance between $\sigma$ and $\tsigma$ as $D_{\eps}(\sigma,\tsigma) = \max_i (\log(\sigma_i) - \log(\tsigma_i))^2$.
\begin{theorem}
