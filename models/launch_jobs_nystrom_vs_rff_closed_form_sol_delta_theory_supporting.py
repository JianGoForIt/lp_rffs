...
# example
#python launch_jobs_nystrom_vs_rff_closed_form_sol_delta_theory_supporting.py census fixed_design/theory_supporting dawn with_metric cpu dryrun fixed_n_feat &
#python launch_jobs_nystrom_vs_rff_closed_form_sol_delta_theory_supporting.py census fixed_design/theory_supporting_fixed_mem dawn with_metric cpu dryrun fixed_memory &
		...
		for n_fp_rff in n_fp_rff_list:
			for approx_type in ["cir_rff"]:
				for n_bit in feat_bit_list:
					...
					if fixed_memory == "fixed_memory":
						command = command.replace("--n_fp_rff=unk", "--n_fp_rff="+str(int(n_fp_rff * 32 / float(n_bit) ) ) )
					else:
