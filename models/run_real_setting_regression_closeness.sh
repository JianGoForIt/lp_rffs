# exact kernel optimal lambda, fixed design setting
# label noise sigma     lambda
# 1e3                   1.76055378e-06
# 1e4                   0.00431012
# 1e2                   0.0
# 1e5                   1.04123522
for lambda in 1e-3 #1e3 1e2 1e5
  do
    python run_model.py --model=ridge_regression \
      --l2_reg=${lambda}  --kernel_sigma=28.8675134595 --random_seed=1 \
      --data_path=/dfs/scratch0/zjian/data/lp_kernel_data/census --save_path=/dfs/scratch0/zjian/lp_kernel/closeness/regression_real_settting/exact_l2_reg_${lambda} --approx_type=exact \
